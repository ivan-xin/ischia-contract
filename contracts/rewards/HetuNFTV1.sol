...
import "../Interfaces/IHetuNFT.sol";

contract HetuNFTV1 is Ownable, IERC721, ERC165, IERC721Metadata,IHetuNFT{
     ...
     */
    modifier onlyMinter() {
        require(_minters[msg.sender], "HetuNFT: must be minter");
        _;
    }
    ...
    modifier onlyTransferable() {
        require(_transferable, "HetuNFT: must transferable");
        _;
    }
    ...
    returns (bool)
    {
        ...
        interfaceId == type(IHetuNFT).interfaceId ||
        super.supportsInterface(interfaceId);
    }

    /**
     * check if it's a minter
     */
    ...
    /**
     * @dev check this contract is allowed nft transfer.
     */
    ...
    /**
     * @dev Get  NFT campaginId.
     */
    function campaginId(uint256 tokenId) public view override returns (uint256) {
        require(_exists(tokenId), "HetuNFT: HetuNFT does not exist");
    ...
    returns (uint256)
    {
        require(account != address(0), "HetuNFT: mint to the zero address");
        ...
        require(
            ...
            "HetuNFT: transfer to non ERC721Receiver implementer"
        );
    ...
    function burn(address account, uint256 id) external override onlyMinter {
        require(
            ...
            "HetuNFT: caller is not approved or owner"
        );
        require(isOwnerOf(account, id), "HetuNFT: not owner");
    ...
    onlyMinter
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            require(isOwnerOf(account, tokenId), "HetuNFT: not owner");
            // Clear approvals
